...
2. The visual workflow installed DejaVu and asserted it for generic families, but the committed canonical Linux pins were rendered with `Arial Narrow` resolving to URW Nimbus Sans Narrow and monospace resolving to Nimbus Mono. Consecutive generated runs were stable but differed from all pins through font metrics and page height. CI now installs and asserts the canonical URW family instead of re-baselining screenshots.
