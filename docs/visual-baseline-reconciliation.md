...
The workflow had changed the Linux package/assertion to DejaVu. Ringcraft CSS requests `Arial Narrow`; the canonical pins resolve it to URW `NimbusSansNarrow-Regular`, while generic monospace resolves to `NimbusMono`. DejaVu changed wrapping and accumulated page height.
...
- `fonts-urw-base35`; `fc-match "Arial Narrow"` must report Nimbus Sans Narrow and `fc-match monospace` must report Nimbus Mono.
